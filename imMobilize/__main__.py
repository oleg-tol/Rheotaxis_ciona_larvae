...
# from qt5_camera.camera_widget import  CameraWidget
...
class imMobilize(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None, *args):
        ...
        self.setWindowTitle("imMobilize Vertical")
        ...
        self.ui.sliderIRBottom.sliderReleased.connect(self.set_IR_bottom)
        self.ui.sliderIRLeft.sliderReleased.connect(self.set_IR_left)
        self.ui.sliderIRRight.sliderReleased.connect(self.set_IR_right)
    ...
    def detect_cameras(self):
        ...
        for cam in self.available_cameras:
            self.ui.comboBoxConnectedCameras.addItem(f"{cam} - {self.available_cameras[cam]}")
    ...
    def camera_connect_disconnect(self):
        if not hasattr(self, "cam") or  not self.cam.cap.isOpened():
            ...
            camera_number = self.ui.comboBoxConnectedCameras.currentText().split(" ")[0]
            ...
            if self.cam.cap.isOpened():
                ...
                self.ui.labelCameraConnectionStatus.setText(f"Connected to {camera_number}")
...
#                print("Signals have been conneted. Maybe. I dunno")
    ...
    def set_experiment_view(self):
        ...
        for ii in range(len(self.Stims.df)):
            ...
            if self.Stims.df.message_on.iloc[ii].startswith("sl"):
                box = LinearRegionItem(values=(start, stop), brush=(255, 255, 250, 230), movable=False)
                self.ui.graphicsView.addItem(box)
    ...
    def serial_connect_disconnect(self):
        ...
        if self.ui.buttonSerialConnect.text().lower() == "connect":
            ...
            if self.arduino.ser.isOpen():
                ...
                for s in [self.ui.sliderIRBottom, self.ui.sliderIRLeft, self.ui.sliderIRRight]:
                    s.setEnabled(True)
    ...
    def toggle_preview(self, event):
        if event:
            ...
            self.ui.labelPreview.setVisible(True)
            self.ui.groupBoxVideoControls.setVisible(False)
            ...
            self.cam.stop()
            time.sleep(0.01)
            self.ui.labelPreview.setText("Duration")
            self.ui.labelPreview.setVisible(False)
            self.ui.groupBoxVideoControls.setVisible(True)

    @QtCore.pyqtSlot(np.ndarray)
    def update_image(self, frame):
        qt_img = self.convert_cv_qt(frame)
        self.ui.labelPreview.setPixmap(qt_img)
...
#        gamma = gamma * 10
    ...
    def set_IR_bottom(self):
        val = self.ui.sliderIRBottom.value()
        self.arduino.write(f"ir6{val}")
    def set_IR_left(self):
        val = self.ui.sliderIRLeft.value()
        self.arduino.write(f"ir3{val}")
    def set_IR_right(self):
        val = self.ui.sliderIRRight.value()
        self.arduino.write(f"ir5{val}")

    def set_path(self):
        self.path = QtGui.QFileDialog.getExistingDirectory(options=QtGui.QFileDialog.DontUseNativeDialog)
        print(self.path)

    def set_working_directory(self):
        self.working_directory_selected = False
        while not self.working_directory_selected:
            self.path = QtGui.QFileDialog.getExistingDirectory(self, "Select a directory", "~/", options=QtGui.QFileDialog.DontUseNativeDialog)
...
#                self.ui.buttonStartExperiment.setChecked(False)
            ...
            df["infrared"] = [{"bottom": self.ui.sliderIRBottom.value(),
                               "left": self.ui.sliderIRLeft.value(),
                               "right": self.ui.sliderIRRight.value()}]
    ...
    def closeEvent(self, event):
        ...
        if QtWidgets.QMessageBox.question(self, "closing", "Are you sure you want to close imMobilize?") == QtWidgets.QMessageBox.Yes:
            try:
                self.arduino.disconnect()
            except:
                print("No controller found to close")
            try:
                self.cam.alive = False
                self.cam.cap.release()
            except:
                print("No camera found to close")

            event.accept()
            print("yay")
        else:
            print("We are not yet going home.")
            event.ignore()
...
if __name__ == "__main__":
    ...
    window = imMobilize()
    window.setWindowTitle("imMobilize Vertical")
    window.show()
